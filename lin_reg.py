...
def create_eqexample(chaincode, num_vals):
	...
	time_limit = num_vals
	user_name = 'user1'
	cmd_list = ['peer','chaincode', 'invoke','-C' ,'mychannel' , '-c','{"Args":["createInstance", "' + eqexample_name + '","'  + '7' +  '","' + user_name + '"  ]}','-n' ,chaincode ,'--peerAddresses', 'peer0.org1.example.com:7051', '--peerAddresses', 'peer1.org1.example.com:7051', '--peerAddresses','peer0.org2.example.com:7051','--peerAddresses', 'peer1.org2.example.com:7051']
	task = subprocess.run(cmd_list, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	out = task.stdout
	err = task.stderr	
	out_str = err.decode('utf-8')
...
def main():
	...
	create_eqexample(chaincode_name, '8')
	time.sleep(2)
